        ...
        cell: ({ row }) => {
          const buttons = [
            ...
            ,(row.original.status == 'Pending' || row.original.status == 'Approved') && {
              id: row.original.id,
              name: row.original.status === 'Approved' ? 'Chat' : 'Approve',
              type: 'pay',
              onClick: () => {
      ...
      try {
        ...
        if (response.data?.status) {
          ...
          if (confirmError) {
            ...
            setSuccess(false);
          } else if (paymentIntent.status === true) {
            const applicationData = {
              applicationId: applicationId,
              amount: projectAmount.total_amount, // Convert back to dollars if needed
              paymentIntentId: paymentIntent.id,
              paymentStatus: paymentIntent.status,
              paymentDetails: paymentIntent, // Send full object if your backend can handle it
            };
            const response = await apiRequest("post", "/update-application-status", applicationData, {
              Authorization: `Bearer ${token}`,
            });
            ...
            if(response.data?.status){
              ...
              modalInstance?.hide(); // ✅ Correctly hide the open modal
          ...
          <div className="modal-content">
            <form onSubmit={handleSubmit}>
              ...
              <div className="modal-body">              
                ...
                >
                  Pay {projectAmount.total_amount}
                </button>                
              </div>             
            </form>
          </div>
